...
function ItemEquipStatsRadarChart({
  ...
  useEffect(() => {
    ...
    stats.map(stat => {
      if (stat == "Cost") {
        if (inspectItem[stat.toLowerCase()] == 0 || inspectItem[stat.toLowerCase()] == undefined || inspectItem[stat.toLowerCase()] == NaN) {
          dataValue[stat] = 0;
    ...
    let data = [[{
      axis: "Damage",
      value: dataValue.Damage
    }, {
      axis: "Speed",
      value: dataValue.Speed
    }, {
      axis: "Cost",
      value: dataValue.Cost
    }, {
      axis: "Critrate",
      value: dataValue.Critrate
    }, {
      axis: "Durability",
      value: dataValue.Durability
    }]];
    renderData(data);
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "title"
  }, inspectItem.itemname, ":")), /*#__PURE__*/React.createElement("div", {
  ...
  }, isItemSelected && stats.map(stat => /*#__PURE__*/React.createElement("div", {
  ...
  }, inspectItem[stat.toLowerCase()]))), isItemSelected && /*#__PURE__*/React.createElement("button", {
    className: "button",
    onClick: () => {
      window.location.assign("/admin/react_update_item.html?itemid=" + inspectItem.itemid);
    }
  }, "Manage Item")));
  function renderData(data) {
    ...
    let svg = d3.select(itemStatsChart.current).append("svg").attr("width", cfg.w + cfg.margin.left + cfg.margin.right).attr("height", cfg.h + cfg.margin.top + cfg.margin.bottom).attr("class", "radar");
    ...
    }).style("fill", "#FFFFFF").style("stroke", "#00000F").style("fill-opacity", cfg.opacityCircles);
    ...
    }).attr("class", "line").style("stroke", "#4298F5").style("stroke-width", "2px").style("stroke-opacity", 0.25);

    //Append the labels at each axis
    axis.append("text").attr("class", "legend").style("font-size", "11px").attr("text-anchor", "middle").attr("dy", "1em").attr("x", function (d, i) {
    ...
    let radarLine = d3.lineRadial().curve(d3.curveCardinalClosed).radius(function (d) {
    ...
    let blobWrapper = g.selectAll(".radarWrapper").data(data).enter().append("g").attr("class", "radarWrapper");

    //Append the backgrounds
    blobWrapper.append("path").attr("class", "radarArea").style("fill-opacity", 0.2).style("fill", function (d, i) {
      return cfg.color(i);
    }).attr("stroke", "#87CEFA").attr("stroke-opacity", 0).attr("stroke-width", "3px").attr("d", function (d, i) {
      return radarLine(d);
    }).transition() // add transition
    .duration(400) // set transition duration to 0.4s
    .style("fill-opacity", cfg.opacityArea).style("stroke-opacity", 1);

    //Render the bubbles at data points
    blobWrapper.selectAll(".radarCircle").data(function (d, i) {
      return d;
    }).enter().append("circle").attr("class", "radarCircle").attr("r", 0).attr("cx", 0).attr("cy", 0).style("fill", function (d, i, j) {
      return cfg.color(j);
    }).transition() // add transition
    .duration(1000) // set transition duration to 1s
    .style("fill-opacity", 1).attr("r", cfg.dotRadius).attr("cx", function (d, i) {
